...
func RegisterServiceHTTPServer(s *http.Server, srv ServiceHTTPServer) {
	r := s.Route("/")
	r.GET("/resource/v1/directories", _Service_AllDirectory0_HTTP_Handler(srv))
...
func _Service_AllDirectory0_HTTP_Handler(srv ServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in AllDirectoryRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationServiceAllDirectory)
		h := ctx.Middleware(func(ctx context.Context, req any) (any, error) {
			return srv.AllDirectory(ctx, req.(*AllDirectoryRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*AllDirectoryReply)
...
func (c *ServiceHTTPClientImpl) AllDirectory(ctx context.Context, in *AllDirectoryRequest, opts ...http.CallOption) (*AllDirectoryReply, error) {
	var out AllDirectoryReply
	pattern := "/resource/v1/directories"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationServiceAllDirectory))
	opts = append(opts, http.PathTemplate(pattern))
